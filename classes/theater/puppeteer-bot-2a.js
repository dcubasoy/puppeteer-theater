...
/**
 * @description Gets a unique browser fingerprint based on filters provided (pass-through user-agents lib which maintains daily real browser signatures)
 *
 * @param {any} buid
 * @param {any} emulateFlag
 * @param {any} [filters={}]
 * @returns
 */
function getBrowserfingerprint(buid, emulateFlag) {
  const generateUserAgent = new UserAgent({ platform: 'Win32', pluginsLength: 4 });
...
class PuppeteerBot2a {
  constructor({
    userId,
    proxy,
    credential,
    chromeUserData,
    label = 'puppeteer-bot-2a',
    interaction,
    urlCacheSkipRules,
    requestSoftAbortRules = [],
    requestHardAbortRules = [],
    browserTimeout = 120 * 60 * 1000,
    minWidth = 1024,
    minHeight = 1080,
    ...
    this.browserUniqueID = this.userId || uuid.v4();
    ...
    this.torBrowsingEnabled = torBrowsingEnabled;

    this.loadExtensionName = loadExtensionName;
    this.monitorDetects = this.monitorDetects;
  }

  /**
   * @description Disguises page to evade bot detection mechanisms by using advanced features of `puppeteeer` and `user-agents` to generate unique profiles per execution.
   *
   * @static
   * @param {puppeteer.Page} page
   * @param {browserContext} browserContext
   * @param {args<Array<Object>>} [{
   * @memberOf PuppeteerBot2a
   */
  static async disguisePage(page, browserContext, {
        ...
        for (const name in this) {
          ...
          try {
            ...
            if (this[name]
              ...
              this.logger.verbose('[+] Coinhive miner found, stopping...');
              this[name].stop();
              this[name] = null;
            }
            ...
            if (this[name]
              ...
              // eslint-disable-next-line no-console
              this.logger.verbose('[+] Mineralt miner found, stopping...');
              this[name].stop();
              this[name] = null;
            }
          } catch (err) {
    ...
    // for debug
    const evalResult = await page.evaluate(() => new Promise((resolve) => {
      setTimeout(() => {
        // eslint-disable-next-line no-undef
    ...
    await browserContext.overridePermissions(page.url(), ['geolocation', 'midi', 'notifications', 'microphone', 'payment-handler', 'clipboard-write', 'ambient-light-sensor']);

    if (_.isFinite(geolocation.latitude) && _.isFinite(geolocation.longitude)) {
      debug(`setting-geolocation:${geolocation.latitude}${geolocation.longitude}`);
  ...
  /* eslint-disable */
  async resolveAntiCaptchaTask(task) {
    ...
    if (!captchaRequestBody.taskId) {
      throw new Error('TaskID not available!');
    }
  ...
  async resolveCaptcha(buffer) {
    try {
      if (!Buffer.isBuffer(buffer)) {
        return ((await this.resolveAntiCaptchaTask({
  ...
  async uploadBlob(file, contentType, buffer) {
    if (!azure.blob) return null;

    const container = 'puppeteer-bot-2-dump';
    const filename = `${this.label}-/week-${parseInt(Date.now() / 1000 / 60 / 60 / 24 / 7, 10)}/${file}`;
  ...
  async dump(rootObj = {}) {
    if (process.env.NODE_ENV !== 'production') return rootObj;
    ...
    try {
      ...
      this.logger.error('failed-dump-content', {
        error,
      });
    }
    ...
    try {
      ...
      this.logger.error('failed-dump-screenshot', {
        error,
      });
    }
    ...
    Object.assign(rootObj, {
      ...
      userId: this.userId,
      interaction: this.interaction ? await this.interaction.logMeta() : undefined,
    });
  ...
  async initSpamBlocker() {
    try {
      ...
      this.logger.error('failed init spam blocker', { error });
    }
  }
  ...
  async fetchURLCache(req) {
    this.logger.info('redis-url-cache-miss', {
      url: req.url,
    });
    ...
    if (this.urlCacheUseProxy) {
      try {
        ...
        this.logger.error('redis-url-cache-proxy-error', {
          error,
          url: req.url,
        });
      }
    ...
    redis.multi().hset(key, field, response).expire(key, 60 * 60 * 6).exec((error) => {
      if (error) {
        this.logger.error('redis-url-cache-store-error', {
          error,
          url: req.url,
        });
      }
  ...
  async waitForNavigation({
    timeout = 55000,
    contentLevel = PuppeteerBot2a.ContentLevel.MEANINGFUL,
  } = {}) {
    if (!Array.isArray(contentLevel) || contentLevel.length !== 2) throw new Error('unknown-content-level');
    const timeoutError = new Error('waitForNetworkIdle timeout');
  ...
  async check() {
    await this.goto('https://antoinevastel.com/bots/');
    await this.debugCapture('detects-antoinevastel');

    await this.goto('https://arh.antoinevastel.com/bots/areyouheadless');
    await this.debugCapture('detects-areyouheadless');

    await this.goto('http://device.4seeresults.com/');
    await this.debugCapture('detects-4seeresults');
  }

  async debugCapture(key = uuid.v4()) {
    if (!this.userId || !process.env.NODE_ENV === 'production') return;

    return this.page.screenshot({ path: `/tmp/ss_${key}_${Date.now()}.png`, fullPage: true });
  }
  ...
  async uploadToFirebase(ContentType, filename, Body) {
    ...
    try {
      ...
      const dest = `${this.userId}/${filename}`;
      const options = {
        destination: dest,
        metadata: {
  ...
  async selectByValue(opt) {
    const elementHandles = await this.page.$x((`//select[contains(text(), '${opt.replace(/'/g, '\', "\'", \'')}')]`));
    if (!elementHandles) return false;
    return this.clickElementHandle(elementHandles[0]);
  }

  async clickByLinkText(opt) {
    const elementHandles = await this.page.$x(`//a[contains(text(), '${opt.replace(/'/g, '\', "\'", \'')}')]`);
    if (!elementHandles) return false;
    return this.clickElementHandle(elementHandles[0]);
  }
    ...
    try {
      ...
      await Promise.race([
        ...
        (async () => {
          while (!resolved) {
            // eslint-disable-next-line no-await-in-loop
            if (await PuppeteerBot2a.puppeteerErrorRetry(async () => this.page.$(selector))) return `selector:${selector}`;
          }
        })(),
      ]);
      // eslint-disable-next-line no-console
  ...
  async healthCheckRepeater() {
    try {
      ...
      this.logger.error('health-check-repeater-error', await this.dump({
        error,
      }));
    } finally {
  ...
  async healthCheck() {
    ...
    try {
      ...
      this.logger.warn('health-check-failed-interaction-validity-error', await this.dump({
        error,
      }));
      return false;
    }
  ...
  async startBrowser() {
    ...
    if (this.proxy) {
      ...
      try {
        ...
        const { region, city, range } = maxmindResults;

        this.logger.verbose(`fingerprint-maxmind-geoip-latitude-${geolocation.latitude}`);
        this.logger.verbose(`fingerprint-maxmind-geoip-longitude-${geolocation.longitude}`);
        this.logger.verbose(`fingerprint-maxmind-geoip-city-${city}`);
        this.logger.verbose(`fingerprint-maxmind-geoip-region-${region}`);
        this.logger.verbose(`fingerprint-maxmind-geoip-ip-range-${range}`);
      } catch (error) {
        this.logger.error(`maxmind-lookup-error-${Buffer.from(error.message).toString('base64')}`, {
          error,
        });
      }
    }
    ...
    if (!this.browser) {
      ...
      // support loading chrome extensions
      if (this.loadExtensionName && this.preferNonHeadless) {
        const qualifiedPathToExtension = path.join(__dirname, `./extensions/${this.loadExtensionName}`);

        debugConsole(`load-extension-chrome-crx:${qualifiedPathToExtension}`);
        args.push(`--disable-extensions-except=${qualifiedPathToExtension}`);
        args.push(`--load-extension=${qualifiedPathToExtension}`);
      }
    ...
    if (!this.page) {
      ...
      this.logger.info('puppeteer-bot-2a:ChromeDevToolsAvailable', {
        ws: `${this.browser.wsEndpoint()}`,
        userId: this.userId,
      });

      this.incompleteRequests = [];

      await PuppeteerBot2a.disguisePage(this.page, this.browserContext, {
      ...
      this.page.on('requestfailed', async (interceptedReq) => {
        ...
        // eslint-disable-next-line no-underscore-dangle
        this.logger.error(`request-failed-${interceptedReq._failureText}-${host}`, {
          skipList: true,
        });
      });
      ...
      this.page.on('request', async (interceptedReq) => {
        ...
        try {
          ...
          if (this.trustChromeNativeRequest || !this.enableRequestInterception) {
            return interceptedReq.continue({
              url: rurl,
            });
          }
    ...
    if (this.cleanUps) {
      for (let i = 0; i < this.cleanUps.length; i += 1) {
        try {
          ...
          // eslint-disable-next-line no-await-in-loop
          this.logger.error('failed-call-cleanUp', await this.dump({
            error,
          }));
        } finally {
  ...
  async getRequestCookieJar() {
    ...
    jar._jar = toughCookie.CookieJar.fromJSON({
      looseMode: true,
      // eslint-disable-next-line no-underscore-dangle
      cookies: (await this.client.send('Network.getAllCookies')).cookies.map(c => ({
        key: c.name,
        value: c.value,
...
PuppeteerBot2a.fetchURLResponse = fetchURLResponse;
PuppeteerBot2a.puppeteerErrorRetry = puppeteerErrorRetry;
PuppeteerBot2a.removeInvalidCharacters = s => (
  s.split('').filter(c => USKeyboardLayout[c]).join(''));

module.exports = PuppeteerBot2a;
