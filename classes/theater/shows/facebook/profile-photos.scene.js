...
class FacebookProfilePhotosScene extends FacebokJustClickAwareScene.WithSpinner {
  constructor(args) {
    super(Object.assign({
      elementQueries: {
        tabActive: {
          selector: 'a[data-tab-key="photos"]',
        },
        uploadPhotosBtn: {
          selector: '[name="composer_photo[]"]',
        },
      },
    }, args));
  }
  ...
  async play() {
    ...
    await this.elements.uploadPhotosBtn.click();
    ...
    const photosToUpload = (spec.photos || [])
      ...
      for (let i = 0; i < photosToUpload.length; i += 1) {
        ...
        // eslint-disable-next-line no-await-in-loop
        await this.elements.uploadPhotosBtn.upload(a.buffer, {
          prefix: `${a.path.name}-`,
          postfix: a.path.ext,
        });
...
module.exports = FacebookProfilePhotosScene;
